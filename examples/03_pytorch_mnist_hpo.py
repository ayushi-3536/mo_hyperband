...
def train_and_evaluate(config, max_budget, verbose=False, **kwargs):
    ...
    logger.debug(f'Model sample:{model}')
...
def objective_function(config, budget, **kwargs):
    ...
    # Build model
    model = Model(config).to(device)
    logger.debug(f'model sampled:{model}')
    # Optimizer
    optimizer = optim.Adadelta(model.parameters(), lr=config["lr"])
...
def input_arguments():
    ...
    parser.add_argument('--max_budget', type=float, default=7,
    ...
    parser.add_argument('--runtime', type=float, default=1500,
...
def main():
    ...
    mohb = MOHB(f=objective_function, cs=cs, min_budget=args.min_budget,
    ...
    if args.refit_training:
        mohb.logger.info("Retraining on complete training data to compute test metrics...")
        train_set = torchvision.datasets.MNIST(
            root='./data', train=True, download=True, transform=transform
        )
        incumbent = mohb.pareto_configs
        acc = train_and_evaluate(incumbent, args.max_budget, verbose=True,
                                 train_set=train_set, test_set=test_set, device=device)
        mohb.logger.info("Test accuracy of {:.3f} for the best found configuration: ".format(acc))
        mohb.logger.info(incumbent)
