...
class SHBracketManager(object):
    ...
    def __init__(self, n_configs, budgets, bracket_id=None):
        ...
        # A dict to keep a track of Trial which are in progress for every budget
        self.pending_trials = {}

        # A dict to keep a track of Trial which are completed for every budget
        self.completed_trials = {}

        # A dict to keep a track of all Trials for every budget
        # For base rung it is populated randomly with number of trials expected
        # For other higher rungs, it is populated with promoted configs from lower budget
        self.trials = {}
    ...
    def get_pending_trials(self, budget):
        assert budget in self.budgets
        trials = self.trials[budget]
    ...
    def register_job(self, budget, trial):
    ...
    def complete_job(self, budget, trial, fitness):
        ...
        if budget in self.completed_trials:
            ...
            self.completed_trials[budget] = []
            self.completed_trials[budget].append(trial)

        assert trial in self.pending_trials[budget]
        self.pending_trials[budget].remove(trial)
